...
export function aggregateApproveInWork(
  data: any[],
  ...
  const result: Responsible[] = Object.values<Responsible>(
    data.reduce((acc: Record<string, Responsible>, item) => {
      ...
      if (
        item[instanceKey]?.[dateAssigned] && // Проверяем, что дата направления на согласование есть
        !item[instanceKey]?.[dateCompleted] // Проверяем, что дата согласования еще не установлена
      ) {
        acc[name].inWork += 1;
      }
